...
`go-ruby-activejob/activejob` reimplements Ruby's `activejob` in **pure Go, with cgo disabled**. The slice of
...
- any Go program can import `github.com/go-ruby-activejob/activejob` directly, with no Ruby runtime;
...
Because `activejob` is pure Go with cgo disabled, it also compiles to **WebAssembly**
